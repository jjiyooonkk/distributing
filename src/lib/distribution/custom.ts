...
export function distributeCustom(
  ...
  const pinRules = config.rules.filter((r): r is PinRule => r.type === 'pin');
  ...
  const ensureRules = config.rules.filter((r): r is EnsureRule => r.type === 'ensure');
  const excludeRules = config.rules.filter((r): r is ExcludeRule => r.type === 'exclude');
